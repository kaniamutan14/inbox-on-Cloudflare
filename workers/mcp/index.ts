...
export class EmailMCP extends McpAgent<any> {
  ...
  constructor(ctx: any, env: Env) {
    super(ctx, env);
    // Read from wrangler.jsonc vars, fallback to "agentic-inbox" if empty
    this.server = new McpServer({
      name: env.MCP_SERVER_NAME || "agentic-inbox",
      version: "1.0.0",
    });
  }
    ...
    this.server.tool(
      ...
      "Send a reply to an email. Only call after drafting and getting confirmation.",
      {
        ...
        from: z
          ...
          .optional()
          .describe(
            "Optional custom from address alias (must belong to the same domain as mailboxId)",
          ),
        subject: z.string().describe("Subject line"),
        bodyHtml: z.string().describe("The HTML body of the reply"),
      },
